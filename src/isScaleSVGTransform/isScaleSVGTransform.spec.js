...
describe(`$$isScaleSVGTransform`, function () {
  describe(`The function will throw an error when the input value is not a SVGTransform.`, function () {
    it(`Input null.`, function () {
      expect(() => $$isScaleSVGTransform(null)).to.throw();
    });

    it(`Input plain empty object.`, function () {
      expect(() => $$isScaleSVGTransform({})).to.throw();
    });

    it(`Input plain object with type and SVG_TRANSFORM_SCALE.`, function () {
      const obj = {
        type: SVGTransform.SVG_TRANSFORM_SCALE,
        SVG_TRANSFORM_SCALE: SVGTransform.SVG_TRANSFORM_SCALE,
      };

      expect(() => $$isScaleSVGTransform(obj)).to.throw();
    });
  });

  it(`
  The function returns true
  if the SVGTransform's type is same with a SVGTransform.SVG_TRANSFORM_SCALE.
  `, function () {
  ...
  if the SVGTransform's type is not same with a SVGTransform.SVG_TRANSFORM_SCALE.
  `, function () {
